...
def _aggregate_success_profile(filtered_df: pd.DataFrame) -> pd.DataFrame:
    ...
    The old logic depended on exact week counts:
    - 4 weeks = High Retention
    - 3 weeks = Balanced
    - everything else = Hype

    This version always tries to pick three archetypes:
    ...
    profile_df["longevity"] = pd.to_numeric(
        profile_df["longevity"],
        errors="coerce",
    )
...
def build_success_profile_figure(profile_df: pd.DataFrame):
    ...
    # X-axis arrow: from origin to the right
    fig.add_annotation(
        x=x_axis_max,
        y=0,
        ax=0,
    ...
    # Y-axis arrow: from origin to the top
    fig.add_annotation(
        x=0,
        y=y_axis_max,
        ax=0,
...
def _render_success_explainer() -> None:
    """Render the storytelling explanation above the filters."""
    st.markdown(
        """
        <div style="
            margin-top: 0.4rem;
            ...
            line-height: 1.5;
        ">
            <strong>Weeks</strong> = time in Top 10. 
            <strong>Score</strong> = how popular the show was.<br>
            <span style="color:#2A9D8F;"><strong>Green</strong></span> = strongest retention / people keep watching again and again. 
            <span style="color:#F4A261;"><strong>Orange</strong></span> = balanced success: strong popularity and strong retention. 
            <span style="color:#E63946;"><strong>Red</strong></span> = strongest hype / highest popularity score.
        </div>
        """,
